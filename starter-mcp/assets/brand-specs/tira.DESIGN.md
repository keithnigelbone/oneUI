...
Tira. Balanced layouts, balanced expression. `#777AD9` is the single interaction driver; the rest of the palette is chrome and status.
...
JioType Variable carries the full hierarchy; JetBrains Mono is reserved for code surfaces. Weights follow the emphasis system: display/headline/title are always 800+; body defaults to 400 (low emphasis); label uses 500 (medium) for UI chrome. Line heights are relational — generated from dimension f-steps so platform and density can shift the whole scale at once. Always pair a size token with its matching line-height token.
