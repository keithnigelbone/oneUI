# PRD Template — design (for later implementation)

> **Status:** designed, NOT yet implemented. Captures the proposed PRD-template format the
> MCP should ship so users have a consistent, complete brief to start building from. Implement
> after the current bug-fix pass.

## Why

A PRD template gives the coding agent a consistent, complete brief so the build is
deterministic. Each section maps to a specific MCP step (invariants → skills → components →
validate → self-heal). The scope split (v1 vs deferred) is what stops the agent from
over-building.

## The template (blank — what the user fills in)
...
- Tokens only, no literals; WCAG AA complaint; responsive.

## 8. Acceptance criteria

- Done when … (renders, passes validate_oneui_code, every button navigates, theme colors accuracy)
```

## How each section drives the MCP

| Section           | MCP step it feeds                                                                                           |
| ----------------- | ----------------------------------------------------------------------------------------------------------- |
| 2. Brand & theme  | `get_brand_tokens` / `list_brands`; theme wiring (`data-theme`)                                             |
| 3 / 3a. Scope     | Keeps the agent from over-building; v1 = wire the flow, defer the rest                                      |
| 4. Screens & flow | `oneui-design-composition` + `surface` skills; `list_components` / `get_component_info` per screen; routing |
| 6. References     | Figma/screenshots ground the layout                                                                         |
| 7. Constraints    | `get_core_invariants`; `validate_oneui_code` (incl. icon/font/surface rules)                                |
| 8. Acceptance     | The self-heal loop's stop condition                                                                         |

## Worked example (the e-commerce prompt, filled in)
