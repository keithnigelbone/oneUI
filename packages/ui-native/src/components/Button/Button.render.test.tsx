...
describe('Button (native)', () => {
  ...
  it('lets recipe cornerRadius override family shapeLanguage', () => {
    render(
      <OneUINativeThemeProvider
        theme={defaultNativeTheme()}
        componentThemeOverrides={{ button: { shapeLanguage: 'pill' } }}
        recipeOverrides={{ button: { cornerRadius: 'none' } }}
      >
        <Button>Recipe Wins</Button>
      </OneUINativeThemeProvider>,
    );
    const flat = StyleSheet.flatten(screen.getByRole('button').props.style);
    expect(flat.borderRadius).toBe(0);
  ...
  it('lets manual borderRadius token override recipe and family selections', () => {
    ...
    render(
      <OneUINativeThemeProvider
        ...
        componentThemeOverrides={{
          button: {
            shapeLanguage: 'sharp',
            tokenRefs: { borderRadius: 'Shape-4' },
          },
        }}
        recipeOverrides={{ button: { cornerRadius: 'small' } }}
      >
        <Button>Manual Wins</Button>
      </OneUINativeThemeProvider>,
    );
    const flat = StyleSheet.flatten(screen.getByRole('button').props.style);
    expect(flat.borderRadius).toBe(theme.shape['4']);
  });
});
