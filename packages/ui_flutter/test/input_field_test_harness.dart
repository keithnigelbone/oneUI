/// Shared harness for [OneUiInputField] widget + semantics tests (web + mobile).
library;
...
const List<TargetPlatform> kOneUiInputFieldTestPlatforms = [
  TargetPlatform.android,
  TargetPlatform.iOS,
  TargetPlatform.linux,
];

NativeDesignSystemPayload inputFieldTestDesignSystem({
  String fieldGapPx = '6px',
  String feedbackGapPx = '4px',
}) {
  final props = <String, dynamic>{
    '--InputField-gap': fieldGapPx,
    '--InputField-labelGap': '2px',
    '--InputField-labelIconGap': '4px',
...
NativeTypographySnapshot inputFieldTestTypography() {
...
ThemeConfig inputFieldTestThemeConfig() {
...
Widget pumpInputFieldApp(
  Widget child, {
  NativeDesignSystemPayload? designSystem,
  bool brandLoading = false,
  String brandHash = 'test-brand',
}) {
  final ds = designSystem ?? inputFieldTestDesignSystem();
...
Future<void> pumpInputFieldHarness(
  WidgetTester tester,
  Widget child, {
  NativeDesignSystemPayload? designSystem,
}) async {
  await tester.pumpWidget(pumpInputFieldApp(child, designSystem: designSystem));
  await tester.pumpAndSettle();
}

Finder inputFieldTextFieldFinder() {
  return find.byWidgetPredicate(
    (w) => w is Semantics && (w.properties.textField ?? false),
  );
}

SemanticsNode inputFieldTextFieldSemantics(WidgetTester tester) {
  return tester.getSemantics(inputFieldTextFieldFinder().first);
}
...
void testWidgetsAllPlatforms(
  ...
  for (final platform in kOneUiInputFieldTestPlatforms) {
