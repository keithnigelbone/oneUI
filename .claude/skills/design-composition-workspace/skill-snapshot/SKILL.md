...
Is this a card or panel?
  ...
  -> minimal (alternating rows or sidebar needing subtle differentiation)
  -> subtle (grouped section needing clear visual boundary)
  -> moderate (stronger emphasis without going bold)

Is this a hero or branded moment?
  -> <Surface mode="bold"> (brand accent surface + automatic child component adaptation)
...
### 7 Surface Tokens — When to Use Each
...
|------|------------|----------|---------|-----------|
| `default` | Page surface (2500 light / 100 dark) | Page bg, cards, headers, product grids | Main layout, product cards | 70-80% |
...
| Inputs, selects | `var(--Shape-3XS)` (f-4) | 8px | Subtle rounding, container feel |
| Small cards | `var(--Shape-M)` (f0) | 16px | Moderate rounding |
| Large containers | `var(--Shape-L)` to `--Shape-2XL` | 18-24px | Gentle rounding |
| FAB | `var(--Shape-M)` (f0) | 16px | Rounded square, not pill |
| Modals/sheets | `var(--Shape-L)` or larger | 18px+ | Soft corners |
| Sharp edges | `var(--Shape-None)` | 0px | Dividers, full-bleed sections |
