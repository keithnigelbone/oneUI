...
|-------|------------|---------|
| `default` | 2500 (light) or 100 (dark) | Page background |
...
**Key insight**: `dir` (contrast direction) is computed dynamically — toward lighter (2500) or darker (100) for maximum contrast against the parent. Bold uses the scale's `baseStep` or `darkerBaseStep` depending on whether the parent is in the light or dark half of the scale (threshold: step 1300).
...
Interaction states shift from a base surface step toward the contrasting direction:

| Token | Base Surface | Offset |
|-------|-------------|--------|
| `hover` | parent | +1 step |
| `pressed` | parent | +2 steps |
| `boldHover` | bold | +1 step |
| `boldPressed` | bold | +2 steps |
