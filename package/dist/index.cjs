...
__export(index_exports, {
  OneuiWebpackPlugin: () => OneuiWebpackPlugin,
...
// src/index.ts
var TAG4 = "[@oneui/webpack-plugin]";
var VIRTUAL_MODULE_DIR = "webpack-virtual";
var VIRTUAL_INDEX_FILE = "oneui-brands.js";
function virtualModulesRoot(cacheDir) {
  return (0, import_node_path5.join)(cacheDir, VIRTUAL_MODULE_DIR);
}
function virtualIndexPath(cacheDir) {
  return (0, import_node_path5.join)(virtualModulesRoot(cacheDir), VIRTUAL_INDEX_FILE);
}
function virtualBrandPath(cacheDir, slug) {
  return (0, import_node_path5.join)(virtualModulesRoot(cacheDir), "oneui-brand", `${encodeURIComponent(slug)}.js`);
}
function virtualThemePath(cacheDir, parent, sub) {
...
function virtualRequestToPath(resolved, request) {
...
var virtualWritesByCacheDir = /* @__PURE__ */ new Set();
function writeVirtualModuleFiles(resolved, manifest) {
  ...
  (0, import_node_fs7.rmSync)(root, { recursive: true, force: true });
  ...
  ensureDir4((0, import_node_path5.dirname)(virtualIndexPath(resolved.cacheDir)));
  (0, import_node_fs7.writeFileSync)(virtualIndexPath(resolved.cacheDir), indexSource);
  for (const slug of Object.keys(resolved.brands)) {
    ...
    ensureDir4((0, import_node_path5.dirname)(file));
    (0, import_node_fs7.writeFileSync)(file, src);
    for (const themeSlug of resolved.brands[slug].themes) {
      ...
      ensureDir4((0, import_node_path5.dirname)(subFile));
      (0, import_node_fs7.writeFileSync)(subFile, subSrc);
    }
  }
}
function installVirtualModuleResolver(compiler, resolved) {
...
var OneuiWebpackPlugin = class {
  constructor(options = {}) {
    this.options = options;
  }
  synced = false;
  apply(compiler) {
    const resolved = resolveOptions(this.options, compiler.context, TAG4);
