// src/index.ts
import { existsSync as existsSync7, mkdirSync as mkdirSync4, rmSync as rmSync4, writeFileSync as writeFileSync3 } from "fs";
...
function virtualModulesRoot(cacheDir) {
  return join3(cacheDir, VIRTUAL_MODULE_DIR);
}
function virtualIndexPath(cacheDir) {
  return join3(virtualModulesRoot(cacheDir), VIRTUAL_INDEX_FILE);
}
function virtualBrandPath(cacheDir, slug) {
  return join3(virtualModulesRoot(cacheDir), "oneui-brand", `${encodeURIComponent(slug)}.js`);
}
...
function writeVirtualModuleFiles(resolved, manifest) {
  ...
  ensureDir4(dirname2(virtualIndexPath(resolved.cacheDir)));
  writeFileSync3(virtualIndexPath(resolved.cacheDir), indexSource);
  for (const slug of Object.keys(resolved.brands)) {
    ...
    ensureDir4(dirname2(file));
    writeFileSync3(file, src);
    for (const themeSlug of resolved.brands[slug].themes) {
      ...
      ensureDir4(dirname2(subFile));
      writeFileSync3(subFile, subSrc);
    }
  }
}
