...
/**
 * Static foundation so `--Primary-*` resolves to a purple primary (close to
 * Figma file `rgb(57, 0, 173)` on bold buttons). Mirrors the shape used in
 * `useBrandCSS` tests — no Convex required.
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const FIGMA_PARITY_FOUNDATION: Record<string, any> = {
  color: {
    config: {
      brandScales: [
...
export function FigmaParityBrandRoot({ children }: { children: ReactNode }) {
  const { cssContent } = useBrandCSS({
    foundationData: FIGMA_PARITY_FOUNDATION,
    theme: 'light',
