/**
 * Live Convex brand injection — adapted verbatim from
 * apps/qa-playground/src/shell/QaConvexBrandInjector.tsx.
 *
 * Resolves a brand (+ optional sub-brand) from Convex foundations, runs the
 * engine via useBrandCSS, and injects the resulting `@layer brand` CSS globally
 * onto :root (id="oneui-foundation-tokens"). Because injection is global, both
 * the chrome and the canvas-rendered One UI components — including nested
 * `data-surface` blocks — resolve their tokens via the CSS cascade.
 */
...
} from '@oneui-ui-internals/utils/buildComponentOverrideCSS'

const MAIN_STYLE_ID = 'oneui-foundation-tokens'
const DIMENSION_STYLE_ID = 'tldraw-playground-dimension-tokens'
const COMPONENT_STYLE_ID = 'tldraw-playground-component-tokens'
...
export function BrandInjector({
  ...
  const baseFoundationData = useQuery(api.foundations.getBrandOverviewData, {
  ...
  const platformsConfig = foundationData?.platforms?.config as
    | PlatformsFoundationConfig
    | undefined
  ...
  return (
    <BrandLogoContext.Provider value={brandLogoValue}>
      <DecorationProvider decorations={decorationMap}>
        <ComponentOverrideInjector brandId={typedBrandId} />
        {children}
      </DecorationProvider>
...
function ComponentOverrideInjector({ brandId }: { brandId: Id<'brands'> }) {
