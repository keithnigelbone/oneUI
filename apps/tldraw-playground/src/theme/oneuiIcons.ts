/**
 * Registers the Jio semantic-icon loader (same pipeline as Storybook /
 * qa-playground) so One UI components that reference semantic icons resolve.
 * Mirrors apps/qa-playground/src/shell/initQaJioIcons.ts.
 *
 * Requires apps/platform/public/jio-icons-data.json (pnpm build:jio-icons).
 */
...
function loadJioIcon(name: string): Promise<IconComponent | null> {
...
/** Call once before React renders any One UI `<Icon />` (see main.tsx). */
export function initOneUiJioIcons(): void {
  if (registered) return
  setJioIconLoader(loadJioIcon)
  registered = true
}
