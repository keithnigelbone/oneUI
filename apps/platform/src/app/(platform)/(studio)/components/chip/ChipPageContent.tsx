/**
 * components/chip/ChipPageContent.tsx
 *
 * Chip component showcase page with Advanced Mode token editor.
...
import chipMachineDocsJson from '@/generated/component-docs/chip.docs.json';
import { useMigratedPlatformsConfig } from '@/hooks';

const chipMachineDocs = chipMachineDocsJson as ComponentDocumentationSpec;
...
/**
 * Chip defaults to `appearance="secondary"`, so its surface-context demo
 * uses secondary-scoped surface fills. We use the root-only
 * `--Secondary-Fill-*` family (same primitive Surface.module.css uses for
 * `--Surface-Fill-*`) because these are NEVER remapped inside
 * `[data-surface]` blocks — so the container always shows the TRUE fill
 * and the chip inside can cascade to its remapped / inverted value
 * without collapsing onto the same colour. See Chip.showcase.tsx for
 ...
 */
function ChipPreview({ tokens }: { tokens: Record<string, string> }) {
  return (
    <div style={{ display: 'flex', gap: 'var(--Spacing-3-5)', flexWrap: 'wrap', ...tokens }}>
      <Chip attention="high" defaultSelected>Bold</Chip>
      <Chip attention="medium" defaultSelected>Subtle</Chip>
      <Chip attention="low" defaultSelected>Ghost</Chip>
    </div>
  );
}
...
function ChipPageInner({
  ...
  const overrideStyles = useMemo(
    () => buildComponentPreviewStyles('Chip', draftOverrides, CHIP_TOKEN_MANIFEST.tokens),
    [draftOverrides]
  );
  const previewTokenStyles = overrideStyles;

  const previewTokenStylesNoColors = useMemo(() => {
    const colorPrefixes = ['--Chip-backgroundColor', '--Chip-textColor', '--Chip-borderColor'];
      ...
      <div className={styles.mainContent} data-panel-open={isOpen}>
        <div className={styles.header}>
          ...
          <p className={styles.description}>
            Interactive toggleable pill element for filtering, selection, and categorization.
            Built on Base UI Toggle for full accessibility (aria-pressed, keyboard nav).
          </p>
        </div>
        ...
        <div className={styles.content} style={{ ...platformTokens }} data-density={previewDensity}>
          ...
          <FoundationCard
            title="Sizes"
            description="Three size options: S (20px), M (24px), L (32px). Typography uses Label role."
          >
            <div className={styles.showcase} style={{ ...allRoleSurfaceVars, ...previewTokenStylesNoColors }}>
              <ChipSizes />
            </div>
          ...
          <FoundationCard
            title="Slots"
            description="Start and end slots accept Icon, Avatar, CounterBadge, and IndicatorBadge. Sizes scale with chip size."
          >
            <div className={styles.showcase} style={{ flexDirection: 'column', gap: 'var(--Spacing-4-5)', ...allRoleSurfaceVars, ...previewTokenStylesNoColors }}>
              <ChipWithSlots />
            </div>
          </FoundationCard>
          ...
          <FoundationCard
            ...
            description="Chips adapt to different surface backgrounds for proper contrast."
          >
            {!tokenSetsBg ? (
                ...
                {SURFACE_MODE_LABELS.map(({ mode, label }) => (
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', gap: 'var(--Spacing-3-5)', flexWrap: 'wrap' }}>
                      ...
                      <Chip appearance="secondary" attention="medium" defaultSelected>Medium</Chip>
                      <Chip appearance="secondary" attention="low" defaultSelected>Low</Chip>
                    </div>
                  </Surface>
                ))}
              </div>
                  ...
                  {configuredRoles.map((role) => {
                    ...
                    const roleVars = roleTokenSet
                      ? {
                          ...
                          // [data-material="transparent"]) applies; hex fallback covers the
                          // FOUC window before brand CSS injects.
                          '--Chip-backgroundColor-bold-selected': `var(--${roleTokenCasing}-Bold, ${roleTokenSet.surfaces.bold.hex})`,
                          '--Chip-textColor-bold-selected': `var(--${roleTokenCasing}-Bold-High, ${roleTokenSet.onBoldContent.high.blendedHex})`,
                        }
                      : {};
                    return (
                      <tr key={role} style={{ ...roleVars } as React.CSSProperties}>
                        <td style={{ ...tdLabel, textTransform: 'capitalize' }}>{role}</td>
                        <td style={tdCenter}><Chip appearance={role as any} attention="high" defaultSelected>{role}</Chip></td>
                        <td style={tdCenter}><Chip appearance={role as any} attention="high">{role}</Chip></td>
                      </tr>
                    );
                  })}
                </tbody>
        ...
        <Suspense fallback={<PropertyPanelSkeleton />}>
          <PropertyPanel
            ...
            renderPreview={(tokens) => (
              <ChipPreview tokens={{ ...tokens, ...previewTokenStyles }} />
            )}
          />
        </Suspense>
      )}
...
/**
 * Chip Component Page — outer shell with Convex data
 */
export default function ChipPage() {
  ...
  const savedOverridesData = useQuery(
    ...
    currentBrand?.id
      ? { brandId: currentBrand.id as Id<'brands'>, componentName: 'chip' }
      : 'skip'
  );
