/**
 * components/checkbox/CheckboxPageContent.tsx
 *
 * Checkbox component showcase page with Advanced Mode token editor.
...
import checkboxMachineDocsJson from '@/generated/component-docs/checkbox.docs.json';
import { useMigratedPlatformsConfig } from '@/hooks';

const checkboxMachineDocs = checkboxMachineDocsJson as ComponentDocumentationSpec;
...
/**
 * Props for CheckboxPageInner
 */
interface CheckboxPageContentProps {
  ...
  /** Full stacking data for per-role checkbox var computation */
 ...
 */
function CheckboxPageInner({
  platformsConfig,
  allRoleSurfaceVars,
  configuredRoles,
  tokenSetsBg,
}: CheckboxPageContentProps) {
  ...
  const overrideStyles = useMemo(
    () => buildComponentPreviewStyles('Checkbox', draftOverrides, CHECKBOX_TOKEN_MANIFEST.tokens),
    [draftOverrides]
  );
  const previewTokenStyles = overrideStyles;

  // Checkbox manifest has no color tokens (only shape/spacing/typography),
  // so no color filtering is needed. Kept for consistency with Button pattern.
      ...
      <div className={styles.mainContent} data-panel-open={isOpen}>
        ...
        <div className={styles.header}>
          ...
          <p className={styles.description}>
            Binary selection with indeterminate support. The <code>appearance</code> role controls border,
            hover, and checked-state fill tokens.
          </p>
        </div>

        {/* Content - Platform dimension overrides only; brand vars are scoped to checkbox previews */}
        <div className={styles.content} style={{ ...platformTokens }} data-density={previewDensity}>
          ...
          <FoundationCard
            title="States"
            description="All visual states including checked, indeterminate, disabled, and read-only."
          >
            <div className={styles.showcase} style={{ ...allRoleSurfaceVars, ...previewTokenStyles }}>
              <CheckboxStates />
            </div>
                  ...
                  {configuredRoles.map((role) => {
                    ...
                    const roleVars = roleTokenSet
                      ? {
                          ...
                          '--_cb-fg-bold': `var(--${roleTokenCasing}-Bold, ${roleTokenSet.surfaces.bold.hex})`,
                          '--_cb-fg-bold-high': `var(--${roleTokenCasing}-Bold-High, ${roleTokenSet.onBoldContent.high.blendedHex})`,
                        }
                      : {};
                    return (
                      <tr key={role} style={{ ...roleVars } as React.CSSProperties}>
                        ...
                        <td style={{ textAlign: 'center', padding: 'var(--Spacing-3)', verticalAlign: 'middle' }}><Checkbox appearance={role as any} checked label={String(role)} /></td>
                        <td style={{ textAlign: 'center', padding: 'var(--Spacing-3)', verticalAlign: 'middle' }}><Checkbox appearance={role as any} checked={false} label={String(role)} /></td>
                      </tr>
                    );
                  })}
                </tbody>
          ...
          <FoundationCard
            ...
            description="Checkbox automatically adapts when placed on different surface backgrounds."
          >
            <CheckboxSurfaceContext />
          </FoundationCard>
          ...
          <FoundationCard
            title="Interactive"
            description="Functional checkboxes with live state management."
          >
            <div className={styles.showcaseColumn} style={{ ...allRoleSurfaceVars, ...previewTokenStyles }}>
              <Checkbox checked={check1} onCheckedChange={setCheck1} label="Subscribe to newsletter" />
              <Checkbox checked={check2} onCheckedChange={setCheck2} label="Enable notifications" />
              <Checkbox checked={check3} onCheckedChange={setCheck3} label="Remember my preferences" />
            </div>
          </FoundationCard>
          ...
          >
            <ComponentDocumentation
              ...
              baselineSpec={checkboxMachineDocs}
              minimal
            />
          </FoundationCard>
        ...
        <Suspense fallback={<PropertyPanelSkeleton />}>
          <PropertyPanel
            ...
            renderPreview={(tokens) => (
              <CheckboxPreview tokens={{ ...tokens, ...previewTokenStyles }} />
            )}
          />
        </Suspense>
      )}
...
export default function CheckboxPage() {
  ...
  // Fetch saved token overrides for checkbox component
  const savedOverridesData = useQuery(
    api.componentTokenOverrides.getComponentOverrides,
    currentBrand?.id
      ? { brandId: currentBrand.id as Id<'brands'>, componentName: 'checkbox' }
      : 'skip'
  );
  ...
  // Fetch recipe selections for checkbox component
