...
export const SIB_AXE_JSON_OUT = join(process.cwd(), 'test-results', 'selectable-icon-button-axe-violations.json');
...
export function writeSibAxeArtefact(results: AxeResults): void {
...
export async function runSibAxePageScan(
  page: Page,
...
export function writeSibAxeHtmlReport(results: AxeResults): void {
  createHtmlReport({
    results,
    options: {
      outputDir: 'test-results',
      reportFileName: 'selectable-icon-button-accessibility-axe-report.html',
      projectKey: 'Selectable Icon Button QA Playground',
      customSummary: 'Selectable Icon Button QA — axe WCAG 2.1 AA. Ingest via pnpm qa:selectable-icon-button:report.',
    },
  });
}
