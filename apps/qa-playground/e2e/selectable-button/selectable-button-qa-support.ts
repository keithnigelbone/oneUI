/**
 * Shared helpers for Selectable Button Playwright automation.
 */
export {
  formatAxeViolations,
  runSelectableButtonAxePageScan,
  seriousOrCritical,
  WCAG_AA_TAGS,
  writeSelectableButtonAxeArtefact,
  writeSelectableButtonAxeHtmlReport,
} from '../selectable-button-playground/axeSelectableButtonPlayground';
export {
  expectFocusVisible,
  rgbaAlpha,
  scrollToSelectableButtonTestId,
  selectableButtonBackgroundRgb,
  selectableButtonBox,
  selectableButtonByTestId,
  selectableButtonSection,
  selectableButtonWrap,
} from '../selectable-button-playground/selectableButtonHelpers';
export { gotoSelectableButtonPlayground } from '../selectable-button-playground/gotoSelectableButtonPlayground';

import { expect, test, type Page } from 'playwright/test';
import {
  clickPageThemeButton as clickPageThemeButtonCore,
  switchPlaygroundToDarkTheme as switchPlaygroundToDarkThemeCore,
} from '../shared/playgroundTheme';

import {
  FIGMA_VALIDATION_TAB,
  SELECTABLE_BUTTON_PLAYGROUND_ROUTE,
  SELECTABLE_BUTTON_ROOT_TESTIDS,
  SELECTABLE_BUTTON_SHOWCASE_AXE_SCOPE,
} from '../selectable-button-playground/manifest';
import { selectableButtonWrap } from '../selectable-button-playground/selectableButtonHelpers';

export {
  FIGMA_VALIDATION_TAB,
  SELECTABLE_BUTTON_PLAYGROUND_ROUTE,
  SELECTABLE_BUTTON_SHOWCASE_AXE_SCOPE,
};

export const SelectableButtonTags = {
  functional: '@Functional',
  accessibility: '@Accessibility',
  smoke: '@Smoke',
} as const;

export const SELECTABLE_BUTTON_TAG_SET = {
  functional: [SelectableButtonTags.functional],
  accessibility: [SelectableButtonTags.accessibility],
  smoke: [SelectableButtonTags.functional, SelectableButtonTags.smoke],
} as const;

const LOG_PREFIX = '[SelectableButton QA]';

export function qaLog(message: string, detail?: Record<string, unknown>): void {
  const extra = detail ? ` ${JSON.stringify(detail)}` : '';
  console.log(`${LOG_PREFIX} ${message}${extra}`);
}

export async function qaStep<T>(title: string, body: () => Promise<T>): Promise<T> {
  qaLog(`STEP: ${title}`);
  return test.step(title, body);
}

export async function openSelectableButtonTestScenarios(page: Page): Promise<void> {
  await qaStep('Navigate to Selectable Button playground', async () => {
    await page.goto(SELECTABLE_BUTTON_PLAYGROUND_ROUTE);
    await expect(page.getByRole('tab', { name: 'Test Scenarios' })).toBeVisible();
  });

  await qaStep('Select the Test Scenarios tab', async () => {
    await page.getByRole('tab', { name: 'Test Scenarios' }).click();
    await expect(page.getByRole('heading', { name: 'Selectable Button', level: 1 })).toBeVisible();
    await selectableButtonWrap(page, SELECTABLE_BUTTON_ROOT_TESTIDS.default).waitFor({
      state: 'visible',
      timeout: 90_000,
    });
  });
}

export async function openSelectableButtonFigmaValidation(page: Page): Promise<void> {
  await qaStep('Open Figma Validation tab', async () => {
    await page.goto(SELECTABLE_BUTTON_PLAYGROUND_ROUTE);
    await page.getByRole('tab', { name: FIGMA_VALIDATION_TAB }).click();
    await expect(page.getByTestId('figma-selectable-button-grid')).toBeVisible();
  });
}

export function attachConsoleErrorCollector(page: Page): string[] {
  const errors: string[] = [];
  page.on('pageerror', (err) => {
    errors.push(`pageerror: ${err.message}`);
  });
  page.on('console', (msg) => {
    if (msg.type() === 'error') errors.push(`console.error: ${msg.text()}`);
  });
  return errors;
}

export async function assertNoConsoleErrors(errors: string[]): Promise<void> {
  expect(
    errors,
    `Browser console should have no errors during Selectable Button scenario load.\n${errors.join('\n')}`,
  ).toHaveLength(0);
}

export async function switchPlaygroundToDarkTheme(page: Page): Promise<void> {
  await qaStep('Switch playground to dark theme', () => switchPlaygroundToDarkThemeCore(page));
}

export async function clickPageThemeButton(page: Page): Promise<{ before: string; after: string }> {
  return qaStep('Toggle mode using the toolbar control', () => clickPageThemeButtonCore(page));
}

export async function expectSectionVisible(page: Page, sectionId: string): Promise<void> {
  const band = page.locator(`[data-section="${sectionId}"]`);
  await band.scrollIntoViewIfNeeded();
  await expect(band, `Story band "${sectionId}" should be visible`).toBeVisible();
}

