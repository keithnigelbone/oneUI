...
export const TOUCH_SLIDER_AXE_JSON_OUT = join(
  process.cwd(),
  'test-results',
  'touch-slider-axe-violations.json',
);
...
export function writeTouchSliderAxeArtefact(results: AxeResults): void {
  mkdirSync(join(process.cwd(), 'test-results'), { recursive: true });
  writeFileSync(
    TOUCH_SLIDER_AXE_JSON_OUT,
    JSON.stringify(
      {
...
export async function runTouchSliderAxePageScan(
  page: Page,
  include = '[data-section^="touch-slider-qa-"]',
): Promise<AxeResults> {
  return new AxeBuilder({ page }).include(include).withTags([...WCAG_AA_TAGS]).analyze();
}

export function writeTouchSliderAxeHtmlReport(results: AxeResults): void {
  createHtmlReport({
    results,
    options: {
      outputDir: 'test-results',
      reportFileName: 'touch-slider-accessibility-axe-report.html',
      projectKey: 'Touch Slider QA Playground',
      customSummary: 'Touch Slider QA — axe WCAG 2.1 AA. Ingest via pnpm qa:touch-slider:report.',
    },
  });
}
