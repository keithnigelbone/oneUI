...
const OUT_DIR = path.join(process.cwd(), 'test-results', 'figma-pipeline-2459-screenshots');
const STYLES_JSON = path.join(process.cwd(), 'test-results', 'figma-pipeline-2459-computed-styles.json');
...
test.describe('Figma 2459:24854 Button pipeline', () => {
  test.beforeAll(async () => {
    await mkdir(OUT_DIR, { recursive: true });
  });

  test('Button QA canvas — regions + representative computed styles', async ({ page }) => {
    await page.goto('/figma-pipeline/button');
    await expect(page.getByRole('heading', { name: /2459:24854/ })).toBeVisible();
