...
export const SSTB_AXE_JSON_OUT = join(process.cwd(), 'test-results', 'selectable-single-text-button-axe-violations.json');
...
export function writeSstbAxeArtefact(results: AxeResults): void {
...
export async function runSstbAxePageScan(
  page: Page,
...
export function writeSstbAxeHtmlReport(results: AxeResults): void {
  createHtmlReport({
    results,
    options: {
      outputDir: 'test-results',
      reportFileName: 'selectable-single-text-button-accessibility-axe-report.html',
      projectKey: 'Selectable Single Text Button QA Playground',
      customSummary: 'Selectable Single Text Button QA — axe WCAG 2.1 AA. Ingest via pnpm qa:selectable-single-text-button:report.',
    },
  });
}
