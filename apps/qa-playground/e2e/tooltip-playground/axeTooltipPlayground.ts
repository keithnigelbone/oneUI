...
export const TOOLTIP_AXE_JSON_OUT = join(process.cwd(), 'test-results', 'tooltip-axe-violations.json');
...
export function writeTooltipAxeArtefact(results: AxeResults): void {
  mkdirSync(join(process.cwd(), 'test-results'), { recursive: true });
  writeFileSync(
    TOOLTIP_AXE_JSON_OUT,
    JSON.stringify(
      {
...
export async function runTooltipAxePageScan(
  page: Page,
  include = '[data-section^="tooltip-"]',
): Promise<AxeResults> {
  return new AxeBuilder({ page }).include(include).withTags([...WCAG_AA_TAGS]).analyze();
}

export function writeTooltipAxeHtmlReport(results: AxeResults): void {
  createHtmlReport({
    results,
    options: {
      outputDir: 'test-results',
      reportFileName: 'tooltip-accessibility-axe-report.html',
      projectKey: 'Tooltip QA Playground',
      customSummary: 'Tooltip QA — axe WCAG 2.1 AA. Ingest via pnpm qa:tooltip:report.',
    },
  });
}
