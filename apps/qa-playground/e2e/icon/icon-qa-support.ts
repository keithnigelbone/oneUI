/**
 * Shared helpers for Icon Playwright automation.
 */
...
import {
  ICON_PLAYGROUND_ROUTE,
  ICON_ROOT_TESTIDS,
  ICON_SHOWCASE_AXE_SCOPE,
  type ICON_DATA_SECTIONS,
} from '../icon-playground/manifest';

export { ICON_PLAYGROUND_ROUTE, ICON_SHOWCASE_AXE_SCOPE };
...
export const ICON_TAG_SET = {
  functional: [IconTags.functional],
  accessibility: [IconTags.accessibility],
  smoke: [IconTags.functional, IconTags.smoke],
} as const;

const LOG_PREFIX = '[Icon QA]';

export const ICON_AXE_SHELL_DISABLE = [
...
export const ICON_AXE_ARTIFACT_JSON = join(process.cwd(), 'test-results', 'icon-axe-violations.json');
...
export async function openIconTestScenarios(page: Page): Promise<void> {
  await qaStep('Navigate to Icon playground', async () => {
    await page.goto(ICON_PLAYGROUND_ROUTE);
    await expect(
  ...
  await qaStep('Select the Test Scenarios tab', async () => {
    ...
    await expect(
      ...
      'Icon page heading should be visible',
    ).toBeVisible();
    await iconRoot(page, ICON_ROOT_TESTIDS.default).waitFor({ state: 'visible', timeout: 90_000 });
  });
}

export function iconRoot(page: Page, testId: string) {
...
export async function assertNoConsoleErrors(errors: string[]): Promise<void> {
  expect(
    errors,
    `Browser console should have no errors during Icon scenario load.\n${errors.join('\n')}`,
  ).toHaveLength(0);
}
...
export async function expectSectionVisible(
  page: Page,
  sectionId: (typeof ICON_DATA_SECTIONS)[number],
): Promise<void> {
...
export function writeAxeHtmlReport(results: AxeResults): void {
  createHtmlReport({
    ...
    options: {
      ...
      reportFileName: 'icon-accessibility-axe-report.html',
      projectKey: 'Icon QA Playground',
      customSummary: `Icon QA Playground — axe WCAG 2.1 AA (${WCAG_AA_TAGS.join(', ')}). Pair with public/qa-reports/icon-summary.json.`,
    },
  });
}

export type ConfigureIconAxeOptions = {
  scopeLabel: string;
  include?: string;
  exclude?: string;
...
export function configureIconAxeBuilder(page: Page, options: ConfigureIconAxeOptions): AxeBuilder {
  let builder = new AxeBuilder({ page }).disableRules([...ICON_AXE_SHELL_DISABLE]);
...
export async function runAxeScan(page: Page, options: ConfigureIconAxeOptions): Promise<AxeResults> {
  return qaStep(`axe: ${options.scopeLabel}`, async () => {
    const results = await configureIconAxeBuilder(page, options).analyze();
