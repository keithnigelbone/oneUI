/**
 * Shared helpers for Icon Button Playwright automation.
 */
...
import {
  ...
  type ICON_BUTTON_DATA_SECTIONS,
} from '../icon-button-playground/manifest';

export { ICON_BUTTON_FIGMA_VALIDATION_TAB, ICON_BUTTON_PLAYGROUND_ROUTE, ICON_BUTTON_SHOWCASE_AXE_SCOPE };

export const IconButtonTags = {
  functional: '@Functional',
  accessibility: '@Accessibility',
  smoke: '@Smoke',
} as const;

export const ICON_BUTTON_TAG_SET = {
  functional: [IconButtonTags.functional],
  accessibility: [IconButtonTags.accessibility],
  smoke: [IconButtonTags.functional, IconButtonTags.smoke],
} as const;

const LOG_PREFIX = '[IconButton QA]';

export const ICON_BUTTON_AXE_SHELL_DISABLE = [
...
export const ICON_BUTTON_AXE_ARTIFACT_JSON = join(
  process.cwd(),
  'test-results',
  'icon-button-axe-violations.json',
);
...
export function iconButtonRoot(page: Page, testId: string) {
  return page.locator(`[data-testid="${testId}"][data-size]`).first();
}

export async function openIconButtonTestScenarios(page: Page): Promise<void> {
  await qaStep('Navigate to Icon Button playground', async () => {
  ...
  await qaStep('Select the Test Scenarios tab', async () => {
    ...
    await expect(
      page.getByRole('heading', { name: 'Icon Button', level: 1 }),
      'Icon Button page heading should be visible',
    ).toBeVisible();
...
export async function openIconButtonFigmaValidationTab(page: Page): Promise<void> {
  await qaStep('Open Icon Button Figma Validation tab', async () => {
    await page.getByRole('tab', { name: ICON_BUTTON_FIGMA_VALIDATION_TAB }).click();
...
export async function assertNoConsoleErrors(errors: string[]): Promise<void> {
  expect(
    errors,
    `Browser console should have no errors during Icon Button scenario load.\n${errors.join('\n')}`,
  ).toHaveLength(0);
}
...
export async function expectSectionVisible(
  page: Page,
  sectionId: (typeof ICON_BUTTON_DATA_SECTIONS)[number],
): Promise<void> {
...
export function writeAxeHtmlReport(results: AxeResults): void {
  createHtmlReport({
    ...
    options: {
      ...
      reportFileName: 'icon-button-accessibility-axe-report.html',
      projectKey: 'Icon Button QA Playground',
      customSummary: `Icon Button QA Playground — axe WCAG 2.1 AA (${WCAG_AA_TAGS.join(', ')}). Pair with public/qa-reports/icon-button-summary.json.`,
    },
  });
}

export type ConfigureIconButtonAxeOptions = {
  scopeLabel: string;
  include?: string;
  exclude?: string;
...
export function configureIconButtonAxeBuilder(
  page: Page,
  options: ConfigureIconButtonAxeOptions,
): AxeBuilder {
  let builder = new AxeBuilder({ page }).disableRules([...ICON_BUTTON_AXE_SHELL_DISABLE]);
  if (options.include) builder = builder.include(options.include);
  if (options.exclude) {
    for (const selector of options.exclude.split(',').map((s) => s.trim()).filter(Boolean)) {
...
export async function runAxeScan(
  ...
  return qaStep(`axe: ${options.scopeLabel}`, async () => {
    const results = await configureIconButtonAxeBuilder(page, options).analyze();
