...
export const INPUT_AXE_JSON_OUT = join(process.cwd(), 'test-results', 'input-axe-violations.json');
...
export function writeInputAxeArtefact(results: AxeResults): void {
...
export async function runInputAxePageScan(page: Page, include = '[data-section^="input-qa-"]'): Promise<AxeResults> {
  return new AxeBuilder({ page }).include(include).withTags([...WCAG_AA_TAGS]).analyze();
}

export function writeInputAxeHtmlReport(results: AxeResults): void {
  createHtmlReport({
    results,
    options: {
      outputDir: 'test-results',
      reportFileName: 'input-accessibility-axe-report.html',
      projectKey: 'Input QA Playground',
      customSummary: `Input QA Playground — axe WCAG 2.1 AA. Ingest via pnpm qa:input:report.`,
    },
  });
}
