...
export const SLIDER_AXE_JSON_OUT = join(process.cwd(), 'test-results', 'slider-axe-violations.json');
...
export function writeSliderAxeArtefact(results: AxeResults): void {
...
export async function runSliderAxePageScan(
  page: Page,
  include = '[data-section^="slider-qa-"]',
): Promise<AxeResults> {
  return new AxeBuilder({ page }).include(include).withTags([...WCAG_AA_TAGS]).analyze();
}

export function writeSliderAxeHtmlReport(results: AxeResults): void {
  createHtmlReport({
    results,
    options: {
      outputDir: 'test-results',
      reportFileName: 'slider-accessibility-axe-report.html',
      projectKey: 'Slider QA Playground',
      customSummary: 'Slider QA — axe WCAG 2.1 AA. Ingest via pnpm qa:slider:report.',
    },
  });
}
