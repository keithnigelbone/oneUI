...
export const STEPPER_AXE_JSON_OUT = join(process.cwd(), 'test-results', 'stepper-axe-violations.json');
...
export function writeStepperAxeArtefact(results: AxeResults): void {
  mkdirSync(join(process.cwd(), 'test-results'), { recursive: true });
  writeFileSync(
    STEPPER_AXE_JSON_OUT,
    JSON.stringify(
      {
...
export async function runStepperAxePageScan(
  page: Page,
  include = '[data-section^="stepper-qa-"]',
): Promise<AxeResults> {
  return new AxeBuilder({ page }).include(include).withTags([...WCAG_AA_TAGS]).analyze();
}

export function writeStepperAxeHtmlReport(results: AxeResults): void {
  createHtmlReport({
    results,
    options: {
      outputDir: 'test-results',
      reportFileName: 'stepper-accessibility-axe-report.html',
      projectKey: 'Stepper QA Playground',
      customSummary: 'Stepper QA — axe WCAG 2.1 AA. Ingest via pnpm qa:stepper:report.',
    },
  });
}
